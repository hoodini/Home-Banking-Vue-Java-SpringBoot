...
public class CardsServicePostNewCardTest {
    ...
    void testSuccessfulDebitCardCreation() {
        ...
        Set<Card> updatedCards = new HashSet<>();
        updatedCards.add(new Card(TypeCard.DEBIT, ColorCard.SILVER, "2555 2254 4554 1234", 456, 
            LocalDateTime.now(), LocalDateTime.now().plusYears(1), testClient));
        testClient.setCards(updatedCards);
        when(clientRepository.findByEmail("john.doe@test.com")).thenReturn(testClient);

        // Act
        ResponseEntity<?> response = cardsService.postNewCard(authentication, TypeCard.DEBIT, ColorCard.SILVER);
    ...
    void testCardCreationMaxCreditCardsReached() {
        ...
        // Act
        ResponseEntity<?> response = cardsService.postNewCard(authentication, TypeCard.CREDIT, ColorCard.SILVER);
    ...
    void testCardCreationMixedCards() {
        ...
        // Add 2 debit cards
        for (int i = 2; i < 4; i++) {
            mixedCards.add(new Card(TypeCard.DEBIT, ColorCard.SILVER, "2555 2254 4554 " + i, 123, 
                LocalDateTime.now(), LocalDateTime.now().plusYears(1), testClient));
        }
        ...
        mixedCards.add(new Card(TypeCard.CREDIT, ColorCard.TITANIUM, "2555 2254 4554 5678", 789, 
            LocalDateTime.now(), LocalDateTime.now().plusYears(1), testClient));
        testClient.setCards(mixedCards);
        when(clientRepository.findByEmail("john.doe@test.com")).thenReturn(testClient);
    ...
    void testCardCreationNullType() {
        ...
        // Act
        ResponseEntity<?> response = cardsService.postNewCard(authentication, null, ColorCard.GOLD);

        // Assert
        assertEquals(HttpStatus.FORBIDDEN, response.getStatusCode());
        assertEquals("You don't specified the type of card, try again ", response.getBody());
        verify(cardRepository, never()).save(any(Card.class));
    }
